...
namespace Carbine.GUI
{
	public class TextRegion : Renderable
	{
		public override Vector2f Position
		{
			...
			{
				this.position = value;
				this.drawText.Position = new Vector2f(this.position.X + (float)this.xCompensate, this.position.Y + (float)this.yCompensate);
			}
		}
		...
		public TextRegion(Vector2f position, int depth, FontData font, string text, int index, int length)
		{
			...
			this.drawText.Position = new Vector2f(position.X + (float)this.xCompensate, position.Y + (float)this.yCompensate);
